...
bool thisIsATOEvent = false;
bool thisIsAWCEvent = false;
...
void checkAtoAwcSched(int i){
    ...
    if (db->isThisEventAtoAwcSet(i, 1)){
      thisIsAWCEvent = true;
      bool awcRunning = awc->doAwc();
      if(!awcRunning){
        mySched->setFlag(i,0);
        int emptyTime = awc->getEmptyTime();
        int fillTime = awc->getFillTime();
      }
    }
    if(!alreadyReset){ //**********************TODO  reset in the doAto and doAwc functions
      mySched->setFlag(i,0);
    }
